...
const TeachersDataPage = () => {
    ...
    const fetchTeachers = async () => {
      ...
      try {
        const response = await fetch(
          "https://courseapi-3kus.onrender.com/api/teachers"
        );
  ...
  const handleDelete = async (registerNumber) => {
    if (window.confirm("Are you sure you want to delete this teacher?")) {
      try {
        const response = await fetch(
          `https://courseapi-3kus.onrender.com/api/teacher/${registerNumber}`,
          {
            method: "DELETE",
          }
        );
        ...
        if (response.ok) {
          setTeachers((prevTeachers) =>
            prevTeachers.filter(
              (teacher) => teacher.registerNumber !== registerNumber
            )
          );
