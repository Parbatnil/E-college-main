...
const Mcateacher = () => {
  ...
  useEffect(() => {
    ...
    setCheck(p);
    axios
      .get("https://courseapi-3kus.onrender.com/api/products?sub=mca")
      .then((res) => {
            ...
            <div className=" container mt-2 mx-3 mb-4">
              <div className=" overflow-hidden rounded-3xl min-h-[550px] sm:min-h-[650px] hero-bg-color flex justify-center">
                ...
                <input type="text" name="video" id="video" className="border-2 border-gray-400 p-2 rounded-md w-80 bg-slate-300" placeholder="Eg.- P.B,R.S" value={teacher} onChange={(e)=>setTeacher(e.target.value)} required /> */}
                    ...
                    <button
                      className="bg-primary text-white bg-orange-500 cursor-pointer hover:scale-105 duration-300 py-2 px-8 rounded-full mt-1"
                      onClick={() => navigate("/mcaliveok@24")}
                    >
                      Live-Class
                    </button>
                    <button
                      className="bg-primary text-white bg-orange-500 cursor-pointer hover:scale-105 duration-300 py-2 px-8 rounded-full mt-1"
                      onClick={() => navigate("/mcateachereaxm@24")}
                    >
                      MCA Exam
                    </button>
                    <h1>Table</h1>
